...
public class EmployeeAggrApp {
	...
	public void aggs(TransportClient client){
		AggregationBuilder aggregation= AggregationBuilders.avg("avg_agg").field(Person.LAST_NAME);

		SearchResponse searchResponse = client.prepareSearch(Person.INDEX).addAggregation(aggregation).execute().actionGet();
		Map<String, Aggregation> aggrMap = searchResponse.getAggregations().asMap();
		for (String key:aggrMap.keySet()){
			Aggregation agg=aggrMap.get(key);
		}
	}
